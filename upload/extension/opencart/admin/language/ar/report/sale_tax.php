<?php
//  Website: WWW.OpenCartArab.com
//  E-Mail : info@OpenCartArab.com

// Heading
$_['heading_title']      = 'تقرير الضرائب';

// Text
$_['text_extension']     = 'الإضافات';
$_['text_edit']          = 'تعديل تقرير الضرائب';
$_['text_success']       = 'تم بنجاح: تم تعديل تقرير الضرائب!';
$_['text_filter']        = 'فلتر';
$_['text_year']          = 'سنوات';
$_['text_month']         = 'أشهر';
$_['text_week']          = 'أسابيع';
$_['text_day']           = 'أيام';
$_['text_all_status']    = 'جميع الحالات';

// Column
$_['column_date_start']  = 'تاريخ البدء';
$_['column_date_end']    = 'تاريخ الانتهاء';
$_['column_title']       = 'عنوان الضريبة';
$_['column_orders']      = 'عدد الطلبات';
$_['column_total']       = 'الإجمالي';

// Entry
$_['entry_date_start']   = 'تاريخ البدء';
$_['entry_date_end']     = 'تاريخ الانتهاء';
$_['entry_group']        = 'التجميع حسب';
$_['entry_order_status'] = 'حالة الطلب';
$_['entry_status']       = 'الحالة';
$_['entry_sort_order']   = 'ترتيب الفرز';

// Error
$_['error_permission']   = 'تحذير: ليس لديك إذن لتعديل تقرير الضرائب!';
